...
class PieceModule extends Module<PieceParams> {
    ...
    override public function composeRules():Map<String, RuleComposition<PieceParams>> {
        return [
            'swap' => {type:Action(new SwapPieceSurveyor(), new SwapPieceActor(), null, null, null), 
                isIncluded:function(p) return !p.allowAllPieces && p.allowSwapping,
            },
            'pick' => {type:Action(new PickPieceSurveyor(), new PickPieceActor(), null, null, function(p) return !p.allowPiecePick), 
                isIncluded:function(p) return !p.allowAllPieces,
            },
            'drop' => {type:Action(new DropPieceSurveyor(), new DropPieceActor(), #if HEADLESS null #else new DropPieceRulePresenter() #end, null, null)},
        ];
    }
